...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //3.
  const existedUser = await User.findOne({  //findOne is inbuilt function for User Model from user.model file
  ...
  if (!avatar) {//checking if uploaded, if no thrwing error by thinking file not uploded by user,
    throw new ApiError(400, "Avatar file is required");
  }
  ...
  //7.
  const createdUser = await User.findById(user._id).select(//mongo DB will assign _id for every created field, so using it to check if the user is created?
    "-password -refreshToken" //this syntax with -before the fields will remove those particular fields from object.
  );

  if (!createdUser) {//if user is not created in DB throwing error.
